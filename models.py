...
get_login_sql = "SELECT username, password FROM Loginstuff WHERE username = %s AND password = %s "
get_customer_sql = "SELECT Customer_ID FROM Customer WHERE User_Name = %s"
update_rating_sql = "INSERT INTO Reviews(Product_Code, Reviews, Customer_ID, Score) VALUES (%s, %s, (SELECT Customer_ID FROM Customer WHERE User_Name = %s), %s)"
...
def login(a, b):
    db_cursor.execute(get_login_sql, (a, b))
    logins = []
...
def get_products_name(name):
    get_products_names_sql = get_products_name_sql + "'%" + name + "%'"
