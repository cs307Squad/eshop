...
<table border='0' >
<form method='post' action='view_cal.php'>
<?php
...
for($i = 0; $i < 32; $i++){
	if(isset($_POST[$i])){
		$value = $i."/".$date['month'];
		setcookie("date", $value, time() + (86400 * 30), "/");
...
for ($d=1;$d<=$lastDay;$d++) {
	...
	if($d == $mday)
		echo("<td class='bg-blue'><a href='#' title='Detail of day'><input type='submit' name='$d' value = '$d' /></a></td>");
	else 
		echo("<td class='bg'><a href='#' title='Detail of day'><input type='submit' name='$d' value = '$d' /></a></td>");
